...
export const messages: Record<Language, Record<string, string>> = {
  'zh-CN': {
    ...
    autoSelectFirstTagLabel: '默认选中第一个标签',
    autoSelectFirstTagHint: '打开 popup 时自动勾选常用标签池的第一个标签',
    globalTagPoolLabel: '常用标签池（逗号分隔）',
    globalTagPoolHint: '作为 popup 标签栏的可选来源',
    domainTagRulesTitle: '网站自动标签（即将上线）',
    domainTagRulesHint: '后续版本将支持根据访问网站自动添加标签，当前请先使用 popup 标签栏手动选择。',
    domainTagRulesDomainLabel: '域名',
    domainTagRulesTagsLabel: '自动添加标签',
    saveSettings: '保存设置',
