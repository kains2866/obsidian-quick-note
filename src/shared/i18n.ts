...
export const messages: Record<Language, Record<string, string>> = {
  'zh-CN': {
    ...
    defaultTagsLabel: 'tags（逗号分隔）',
    defaultTagsPlaceholder: 'quick-note',
    tagBarLabel: 'tags',
    tagAddButton: '+ 添加',
    tagInputPlaceholder: '输入 tags',
    autoSelectFirstTagLabel: '默认选中第一个 tags',
    autoSelectFirstTagHint: '打开 popup 时自动勾选 tags 池的第一个 tag',
    globalTagPoolLabel: 'tags（逗号分隔）',
    globalTagPoolHint: '作为 popup 标签栏的可选来源',
    domainTagRulesTitle: '网站自动 tags',
    domainTagRulesHint: '当访问的网页域名匹配下方规则时，对应的 tags 会自动在 popup 中勾选。',
    domainTagRulesDomainLabel: '域名',
    domainTagRulesTagsLabel: '自动添加 tags',
    addDomainRule: '添加规则',
    editDomainRule: '编辑',
    saveDomainRule: '保存',
    removeDomainRule: '删除',
    invalidDomain: '域名格式不正确',
    emptyDomainOrTags: '域名和 tags 不能为空',
    domainRuleDomainPlaceholder: '例如 bilibili.com',
    domainRuleTagsPlaceholder: '例如 bilibili, 视频笔记',
    saveSettings: '保存设置',
    ...
    editShortcuts: '编辑快捷键',
    settingsSaved: '设置已保存',
    unsavedChangesWarning: '当前设置未保存，确定要离开吗？',
    ...
    guideDefaultTagsSummary: 'tags',
    guideDefaultTagsDetail: '对应 frontmatter 里的 tags，使用逗号分隔多个标签。请注意标签本身不支持空格。',
    guideTagBarSummary: '为什么 popup 里有时看不到 tags 栏？',
    guideTagBarDetail: 'tags 栏是否显示由 Frontmatter 里的 tags 开关控制。只有在 popup 中勾选 tags，或者在设置页默认开启 tags 时，标签栏才会出现。',
    guideTempTagSummary: '临时 tag 和全局 tag 有什么区别？',
    guideTempTagDetail: '在 popup 里点击「+ 添加」新增加的 tag 会显示为浅绿底、虚线边框，表示它只在当前这条笔记生效，不会同步到设置页的常用标签池。来自设置页常用标签池的 tag 显示为白底实线，是全局 tag。',
  ...
  'en': {
    ...
    autoSelectFirstTagLabel: 'Auto-select first tag',
    autoSelectFirstTagHint: 'Automatically check the first tag in the global tag pool when opening the popup',
    globalTagPoolLabel: 'Global tag pool (comma separated)',
    globalTagPoolHint: 'Used as the source for the popup tag bar',
    domainTagRulesTitle: 'Website auto-tags',
    domainTagRulesHint: 'When the visited website domain matches a rule below, the corresponding tags will be auto-selected in the popup.',
    domainTagRulesDomainLabel: 'Domain',
    domainTagRulesTagsLabel: 'Auto-add tags',
    addDomainRule: 'Add rule',
    editDomainRule: 'Edit',
    saveDomainRule: 'Save',
    removeDomainRule: 'Remove',
    invalidDomain: 'Invalid domain format',
    emptyDomainOrTags: 'Domain and tags cannot be empty',
    domainRuleDomainPlaceholder: 'e.g. bilibili.com',
    domainRuleTagsPlaceholder: 'e.g. bilibili, video-notes',
    ...
    guideTagBarSummary: 'Why does the tag bar sometimes not appear in the popup?',
    guideTagBarDetail: 'The tag bar is controlled by the tags switch in Frontmatter. It only appears when tags is checked in the popup, or when tags is enabled by default in the settings page.',
    guideTempTagSummary: 'What is the difference between a temporary tag and a global tag?',
    guideTempTagDetail: 'Tags added via the "+ Add" button in the popup have a light green background and dashed border. They only apply to the current note and are not synced to the global tag pool in settings. Tags from the global tag pool have a white background and solid border.',
